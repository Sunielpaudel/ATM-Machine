...
namespace atmProject{
    ...
    void ATM::loadDataFromFile(const std::string& filename) {
        ...
        while (file >> accNum) {
            file.ignore();  // Ignore the newline left by the previous extraction (for account number)
            ...
            std::cout << "Read Account Type: '" << typeStr << "'\n";

            if (typeStr == "Checking") {
                type = AccountType::Checking;
            }
            else if (typeStr == "Savings" || typeStr == "Saving") {  // Account type could be 'Saving' (typo)
            ...
            std::cout << "Read Account: " << accNum << " | " << holder << " | " << pass << " | " << balance << " | " << typeStr << "\n";
        }
